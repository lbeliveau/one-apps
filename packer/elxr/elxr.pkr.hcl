...
source "qemu" "exlr" {
  ...
  iso_url      = lookup(lookup(var.elxr, var.version, {}), "iso_url", "")
  iso_checksum = lookup(lookup(var.elxr, var.version, {}), "iso_checksum", "")

  headless = var.headless

  http_directory = "${var.input_dir}"
  boot_command   = ["<esc><wait>",
    "/install.amd/vmlinuz initrd=/install.amd/initrd.gz auto=true preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/elxr.preseed hostname=localhost domain=localdomain interface=auto vga=788 noprompt quiet --<enter>"]
  boot_wait      = "10s"
